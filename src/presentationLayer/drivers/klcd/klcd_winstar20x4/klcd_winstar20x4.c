...
/*
	...
	lcd_instruction(0x20);		// Instruction 0010b (Function set)
	lcd_instruction(0xC0);		// Instruction NF**b
					// Set N = 1, or 2-line display
					// Set F = 0, or 5x8 dot character font
...
static void lcd_print(char * msg, unsigned int lineNumber)
{
	...
	if( lineNum == 2 )
	{
		...
		while( *(msg) != '\0' )
		{
			if(counter >=  NUM_CHARS_PER_LINE )
			{
				lineNum = 3;	// continue writing on the next line if the string is too long
				counter = 0;
				break;
			}
	...
	if( lineNum == 3 )
	{
		lcd_setLinePosition( LCD_THIRD_LINE );

		while( *(msg) != '\0' )
		{
			if(counter >=  NUM_CHARS_PER_LINE )
			{
				lineNum = 4;	// continue writing on the next line if the string is too long
				counter = 0;
				break;
			}
	...
	if( lineNum == 4)
	{
		lcd_setLinePosition( LCD_FOURTH_LINE);
...
static void lcd_print_WithPosition(char * msg, unsigned int lineNumber, unsigned int nthCharacter)
{
	...
	if( lineNum == 3 )
	{
		lcd_setPosition( LCD_THIRD_LINE, nthChar );
	...
	if( lineNum == 4)
	{
		lcd_setPosition( LCD_FOURTH_LINE, nthChar );
